...
TEST_CASE("1 buffer, with pack/unpack, on device, same MPI rank") {
    ...
    for(auto size : sizes) {
        ...
        for(auto halowidth : halowidths) {
            ...
            tausch->addLocalHaloInfo(sendIndices, 1, -1, TauschOptimizationHint::CudaStaysOnDevice);
            tausch->addRemoteHaloInfo(recvIndices, 1, -1, TauschOptimizationHint::CudaStaysOnDevice);
...
TEST_CASE("2 buffers, with pack/unpack, on device, same MPI rank") {
    ...
    for(auto size : sizes) {
        ...
        for(auto halowidth : halowidths) {
            ...
            double *cuda_in1, *cuda_in2, *cuda_out1, *cuda_out2;
            ...
            cudaMemcpy(cuda_in1, in1, (size+2*halowidth)*(size+2*halowidth)*sizeof(double), cudaMemcpyHostToDevice);
            cudaMemcpy(cuda_in2, in2, (size+2*halowidth)*(size+2*halowidth)*sizeof(double), cudaMemcpyHostToDevice);
            ...
            tausch->addLocalHaloInfo(sendIndices, 2, -1, TauschOptimizationHint::CudaStaysOnDevice);
            tausch->addRemoteHaloInfo(recvIndices, 2, -1, TauschOptimizationHint::CudaStaysOnDevice);
