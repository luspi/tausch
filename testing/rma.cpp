...
TEST_CASE("1 buffer, with pack/unpack, same MPI rank, RMA") {
    ...
    for(auto size : sizes) {
        ...
        for(auto halowidth : halowidths) {
            ...
            tausch.setSendCommunicationStrategy(0, Tausch::Communication::RMA);
            tausch.setRecvCommunicationStrategy(0, Tausch::Communication::RMA);
...
TEST_CASE("1 buffer, with pack/unpack, multiple MPI ranks, RMA") {
    ...
    for(auto size : sizes) {
        ...
        for(auto halowidth : halowidths) {
            ...
            tausch.recv(0, 0, (mpiRank+mpiSize-1)%mpiSize, false);
