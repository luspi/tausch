...
TEST_CASE("1 buffer, derived MPI datatype, same MPI rank") {
    ...
    for(auto size : sizes) {
        ...
        for(auto halowidth : halowidths) {
            ...
            tausch->addRemoteHaloInfoOCL(recvIndices, 1, -1, TauschOptimizationHint::SenderUsesMpiDerivedDatatype);

            tausch->send(0, 0, rank, in, false);
            tausch->recvOCL(0, 0, rank);
...
//            tausch->addRemoteHaloInfoOCL(recvIndices, 2, -1, TauschOptimizationHint::SenderUsesMpiDerivedDatatype);
...
//            tausch->recvOCL(0, 1, mpiRank);

//            std::cout << "c" << std::endl;

//            tausch->unpackRecvBufferOCL(0, 0, cl_out1);
//            tausch->unpackRecvBufferOCL(0, 1, cl_out2);
