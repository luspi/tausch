...
CTauschUnsignedInt *tausch_new_unsignedint(int *localDim, int numBuffers, int valuesPerPoint, MPI_Comm comm, TauschVersion version) {
    ...
    if(version == TAUSCH_1D)
        t = new Tausch1D<unsigned int>(localDim, MPI_DOUBLE, numBuffers, valuesPerPoint, comm);
    else if(version == TAUSCH_2D)
        t = new Tausch2D<unsigned int>(localDim, MPI_DOUBLE, numBuffers, valuesPerPoint, comm);
    else if(version == TAUSCH_3D)
        t = new Tausch3D<unsigned int>(localDim, MPI_DOUBLE, numBuffers, valuesPerPoint, comm);
