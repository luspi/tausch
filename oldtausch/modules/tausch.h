...
public:
    Tausch(MPI_Datatype mpiDataType, size_t numBuffers = 1, size_t *valuesPerPointPerBuffer = NULL, MPI_Comm comm = MPI_COMM_WORLD, bool duplicateCommunicator = true) {
        tausch1 = new Tausch1D<buf_t>(mpiDataType, numBuffers, valuesPerPointPerBuffer, comm, duplicateCommunicator);
        tausch2 = new Tausch2D<buf_t>(mpiDataType, numBuffers, valuesPerPointPerBuffer, comm, duplicateCommunicator);
        tausch3 = new Tausch3D<buf_t>(mpiDataType, numBuffers, valuesPerPointPerBuffer, comm, duplicateCommunicator);
    }
    ~Tausch() {
        delete tausch1;
        delete tausch2;
...
//    void setRemoteHaloInfo3D_GwG(size_t numHaloParts, TauschHaloSpec *haloSpecs);
    ...
    void postReceive3D_CwC(size_t haloId, int mpitag = -1, int remoteMpiRank = -1) { tausch3->postReceiveCwC(haloId, mpitag, remoteMpiRank); }
#ifdef TAUSCH_OPENCL
...
//    void packSendBuffer3D_GwG(size_t haloId, size_t bufferId, cl::Buffer buf);
    ...
    void send1D_CwC(size_t haloId, int mpitag = -1, int remoteMpiRank = -1) { tausch1->sendCwC(haloId, mpitag, remoteMpiRank); }
    void send2D_CwC(size_t haloId, int mpitag = -1, int remoteMpiRank = -1) { tausch2->sendCwC(haloId, mpitag, remoteMpiRank); }
    void send3D_CwC(size_t haloId, int mpitag = -1, int remoteMpiRank = -1) { tausch3->sendCwC(haloId, mpitag, remoteMpiRank); }
#ifdef TAUSCH_OPENCL
