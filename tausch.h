...
#ifdef TAUSCH_MPI_DERIVE
            MPI_Type_free(&sendDatatype[i])      ;
            MPI_Type_free(&recvDatatype[i]);
#endif
        }
...
#else
    inline MPI_Request *send(size_t haloId, const int msgtag, int remoteMpiRank = -1, const bool blocking = false) {
#endif
